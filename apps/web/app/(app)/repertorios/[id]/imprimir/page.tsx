...
export default async function PrintRepertoryPage({ params }: Props) {
  ...
  // Altura disponível para as colunas:
  // 297mm (A4) - 10mm (topo) - 10mm (base) - ~18mm (header) - ~10mm (footer)
  const COLS_HEIGHT = '249mm';
  ...
  const css = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ═══ PRINT ═══ */
    @page {
      size: 210mm 297mm;   /* portrait explícito */
      margin: 10mm 11mm;
    }
    ...
    #screen-wrap {
      ...
      background:#b0b0b0; overflow-y:auto;
      display:flex; justify-content:center;
      padding:20px 16px 48px;
    }

    /* ═══ FOLHA — largura A4, altura livre na tela ═══ */
    .sheet {
      width: 210mm;
      background: white;
      box-shadow: 0 3px 24px rgba(0,0,0,.35);
      padding: 10mm 11mm 10mm;
      /* sem height fixo aqui — o browser quebra ao imprimir */
    }
    ...
    .rh-title { font-size:11pt; font-weight:bold; color:#1e3a5f; line-height:1.2; font-family:Georgia,serif; }
    .rh-meta  { font-size:6.5pt; color:#666; display:flex; gap:8px; align-items:center; flex-wrap:wrap; font-family:sans-serif; }
    .rh-badge {
    ...
    .cols {
      ...
      /* column-fill:auto + height fixa = browser pagina automaticamente ao imprimir */
      column-fill: auto;
      height: ${COLS_HEIGHT};
      overflow: visible; /* na tela deixa vazar abaixo */
    }
    ...
    /* padding esquerdo na col da direita (alternado via JS seria perfeito,
       mas com CSS puro usamos padding direito + regra central) */
    ...
    .sn {
      ...
      min-width:14px; height:14px; border-radius:50%; flex-shrink:0; margin-top:1px;
      font-family:sans-serif;
    }
    .st { font-size:8.5pt; font-weight:bold; color:#1e3a5f; flex:1; line-height:1.25; font-family:Georgia,serif; }
    .sk {
    ...
    /* ═══ AUTOR ═══ */
    .sa { font-size:6pt; color:#9ca3af; font-style:italic; margin:0 0 3px 18px; font-family:sans-serif; }

    /* ═══ OBSERVAÇÃO DO ITEM ═══ */
    .so {
      font-size:6pt; color:#6b7280; font-style:italic;
      border-left:2px solid #e5e7eb; padding-left:4px; margin-bottom:3px;
      font-family:sans-serif;
    }
    ...
    @media print {
      ...
      .cols {
        /* Na impressão, overflow das colunas vira nova página automaticamente */
        overflow:hidden;
      }
    }
  `;
        ...
        <div className="sheet">
          ...
          <div className="cols">
            {items.map((item:any, idx:number) => {
              ...
              const categoryName: string = song.categories?.[0]?.category?.name ?? '';
