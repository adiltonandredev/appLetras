...
// PDF generation is handled via /imprimir route
import type { Repertory, UserRole } from '@rl/types';
import { CELEBRATION_LABELS, CELEBRATION_ICONS, formatDate, can } from '@rl/utils';
import {
  Edit2, Printer, FileDown, Trash2, Share2, ArrowLeft,
  Music, ChevronRight, Eye, Clock, MapPin,
} from 'lucide-react';
              ...
              <div key={item.id} className="group">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-semibold text-gray-900 truncate">{song?.title ?? '(música removida)'}</p>
                      {song?.categories?.[0] && (
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                      ...
                      {item.observations && (
                        <span className="text-xs text-amber-600 font-medium">⚡ {item.observations}</span>
                      )}
                    </div>
                  </div>
                ...
                {isExpanded && song?.lyrics && (
                  <div className="px-5 pb-5 pt-2 border-t border-gray-50 bg-gray-50/50">
                    <pre className="text-sm text-gray-600 whitespace-pre-wrap font-sans leading-relaxed max-h-60 overflow-y-auto">
                      {song.lyrics}
                    </pre>
                    <div className="flex items-center gap-2 mt-3">
                      <Link
                        ...
                        target="_blank"
                      >
                        <Eye className="w-3.5 h-3.5" /> Ver música completa
                      </Link>
                    </div>
                  </div>
                )}
