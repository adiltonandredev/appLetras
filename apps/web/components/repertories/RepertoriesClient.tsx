...
import { getRepertories, deleteRepertory, duplicateRepertory } from '@rl/api-client';
import { CELEBRATION_ICONS, CELEBRATION_LABELS, formatDate } from '@rl/utils';
import type { Repertory } from '@rl/types';
import { Plus, Search, BookOpen, Calendar, Copy, Trash2, Eye, MoreVertical } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{data?.count ?? 0} repertórios</p>
        </div>
      ...
      {data && data.total_pages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button onClick={() => setPage(p => p - 1)} disabled={page === 1} className="btn-secondary px-3 py-1.5 text-sm disabled:opacity-40">
            Anterior
          </button>
          <span className="text-sm text-gray-500">Página {page} de {data.total_pages}</span>
          <button onClick={() => setPage(p => p + 1)} disabled={page >= data.total_pages} className="btn-secondary px-3 py-1.5 text-sm disabled:opacity-40">
            Próxima
          </button>
        </div>
      )}
...
function RepertoryCard({
  repertory, menuOpen, onMenuToggle, onDuplicate, onDelete,
}: {
  repertory: Repertory;
  menuOpen: boolean;
  onMenuToggle: () => void;
